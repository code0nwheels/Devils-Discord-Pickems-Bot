    ...
    async def run(self):
        ...
        while True:
            ...
            if records_updated.date() == datetime.now().date():
                ...
                # if it's 4:30am EST, post leaderboard
                if est.hour == 4 and est.minute == 30:
                       await self.post_leaderboard()
            # sleep until 4:30am EST the next day
            tomorrow = est.replace(hour=4, minute=30, second=0, microsecond=0) + timedelta(days=1)
            sleep_seconds = (tomorrow - est).total_seconds()
